...
import { Search, Eye, Check, X, Loader2, Download } from "lucide-react"
...
interface PendingResult {
  ...
  programId?: string;
  programName?: string;
  programType?: string; // Degree, Diploma, Certificate, etc.
  studyMode?: string; // Regular, Weekend, etc.
  grades: Grade[];
}
...
function ResultApprovals() {
  ...
  const fetchPendingGrades = async () => {
    try {
      ...
      // Enforce lecturer assignment constraints from Firestore to align to director's setup
      // Load active academic period (years/semesters) and lecturer-assignments so grouping/filtering respects program/year/semester/mode
      const assignmentsRef = collection(db, 'lecturer-assignments')
      const assignmentsSnapshot = await getDocs(assignmentsRef)
      const assignmentIndex = new Map<string, any>()
      assignmentsSnapshot.forEach(a => {
        const d = a.data()
        const key = `${d.courseId}|${d.lecturerId}|${d.academicYearId}|${d.academicSemesterId}|${d.programId}|${d.programmeCourseType}|${d.level}`
        assignmentIndex.set(key, d)
      })

      // Courses/programs lookup for academic year/semester names when needed will be resolved later
      ...
      // Group grades by course and lecturer (assignment awareness handled during enrichment)
  ...
  const groupGradesByCourse = async (grades: Grade[]): Promise<PendingResult[]> => {
    ...
    for (const grade of grades) {
      ...
      if (!grouped[key]) {
        ...
        // Enrich with program and study mode
        ...
        try {
          if (!courseCache[grade.courseId]) {
            ...
            for (const coll of courseRefNames) {
              try {
                ...
                if (courseDoc.exists()) {
                  courseCache[grade.courseId] = courseDoc.data()
                  break
                }
              } catch {}
            }
          ...
          if (courseData) {
            ...
            studyMode = courseData.studyMode || studyMode
          }
          if (programId) {
            ...
            if (progData) {
              ...
              programType = (progData.type || '').toString()
              // Title-case for display
              if (programType) programType = programType.charAt(0).toUpperCase() + programType.slice(1)
            }
          }
        } catch (e) {
          console.warn('Program enrichment failed:', e)
        }

        // Lecturer department enrichment
        let lecturerDepartment: string | undefined
        try {
          if (!lecturerDeptCache[grade.lecturerId]) {
            // Try academic-staff first
            const staffRef = doc(db, 'academic-staff', grade.lecturerId)
            const staffDoc = await getDoc(staffRef)
            if (staffDoc.exists()) {
              lecturerDeptCache[grade.lecturerId] = staffDoc.data().department || staffDoc.data().faculty || ''
            } else {
              const userRef = doc(db, 'users', grade.lecturerId)
              const userDoc = await getDoc(userRef)
              if (userDoc.exists()) {
                lecturerDeptCache[grade.lecturerId] = userDoc.data().department || userDoc.data().faculty || ''
              } else {
                lecturerDeptCache[grade.lecturerId] = ''
              }
            }
          }
    ...
    return sortedGroups.flatMap((group) => [
      ...
      <TableRow key={`header-${group.programType}-${group.programName}-${group.academicYear}-${group.semester}-${group.studyMode}`} className="bg-gray-50">
        <TableCell colSpan={7} className="font-semibold text-gray-700 py-3">
          ...
          <span className="ml-2 text-sm text-muted-foreground">{group.results.length} course{subgroupPlural(group.results.length)}</span>
        </TableCell>
      </TableRow>,
